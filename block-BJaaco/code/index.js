...
for (let num of numbers) collection.push(num);
for (let id of userIds) collection.push(id);
...
for (let num of numbers) {
	if (num % 2 == 0) evenCollection.push(num);
}

for (let id of userIds) {
	if (id % 2 == 0) evenCollection.push(id);
}
...
for (let num of numbers) {
	if (num % 2 != 0) oddCollection.push(num);
}

for (let id of userIds) {
	if (id % 2 != 0) oddCollection.push(id);
}
...
function times(times, character = "test") {
	let arr = [];
	while (times > arr.length) arr.push(character);
	return arr;
}
...
function revert(arr) {
	let revertArr = [];
	for (let num of arr) revertArr.unshift(num);
	return revertArr;
}
...
function clear(arr) {
	...
	for (let value of arr) {
		if (Boolean(value) != false) clearedArr.push(value);
	}
	return clearedArr;
}
